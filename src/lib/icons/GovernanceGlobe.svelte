<script>
    ...
    const AXES = [
        ...
        {
            key: "ocean_stewardship",
            label: "پاراستنی\nدەریا",
            color: "#0C4A6E",
            defaultValue: 0.5,
        },
        {
            key: "democratic_governance",
            label: "ایداره‌ائتمه",
            color: "#1E3A8A",
            defaultValue: 0.72,
        },
        {
            key: "freedom_of_press",
            label: "مطبوعات\nآزادلیغی",
            color: "#78350F",
            defaultValue: 0.5,
        },
        ...
        },
        {
            ...
            label: "سیاسی پدگیری ءِ\nجاگہ",
            color: "#713F12",
            defaultValue: 0.68,
        },
        {
            key: "freedom_of_assembly",
            label: "آزاتی ءِ\nچمّانکی",
            color: "#7C2D12",
            defaultValue: 0.62,
        },
    ...
    function buildSpiralOrder(pts) {
        ...
        while (improved && guard++ < 50) {
            ...
            for (let i = 0; i < n - 1; i++) {
                for (let k = i + 1; k < n; k++) {
                    const a = order[(i - 1 + n) % n],
                        b = order[i];
                    const c = order[k],
                        d = order[(k + 1) % n];
                    if (a === c || b === d) continue;
                    const before =
                        Math.sqrt(dist2(a, b)) + Math.sqrt(dist2(c, d));
                    const after =
                        Math.sqrt(dist2(a, c)) + Math.sqrt(dist2(b, d));
    ...
    function draw() {
        ...
        const sinceDragEnd =
            lastDragEnd !== null ? nowMs - lastDragEnd : Infinity;
        const dragHold = inDrag || sinceDragEnd < DRAG_RESUME_MS;
        const hiIdx = highlightIdx;
        const focusA =
            !dragHold && !hoverFreeze && hiIdx >= 0 && hiIdx < axes3D.length
                ? axes3D[hiIdx]
                : null;
        ...
        if (focusA) {
            ...
            const delta =
                ((((targetYaw - angleY) % TAU) + TAU + Math.PI) % TAU) -
                Math.PI;
            const resumeRamp =
                sinceDragEnd < DRAG_RESUME_MS
                    ? Math.max(
                          0,
                          (sinceDragEnd - DRAG_RESUME_MS * 0.5) /
                              (DRAG_RESUME_MS * 0.5),
                      )
                    : 1;
        ...
        function surfaceHeat(dir) {
            ...
            for (let i = 0; i < axisDirsRot.length; i++) {
                const d =
                    dir.x * axisDirsRot[i].x +
                    dir.y * axisDirsRot[i].y +
                    dir.z * axisDirsRot[i].z;
                if (d > bestDot) {
                    bestDot = d;
                    bestIdx = i;
                }
        ...
        for (const p of sphereDots) {
            ...
            const focusDot = focusDir
                ? dir.x * focusDir.x + dir.y * focusDir.y + dir.z * focusDir.z
                : 1;
            ...
            if (heat > 0.02) {
                const alpha =
                    Math.min(0.95, 0.25 + heat * 1.4) *
                    (0.5 + depth * 0.5) *
                    focusMul;
                const radius =
                    (0.6 + heat * 1.75 + depth * 0.4) * (0.5 + 0.5 * focusMul);
                ctx.beginPath();
                ctx.arc(pr.x, pr.y, radius, 0, TAU);
                ctx.fillStyle = hexRGBA(axisColors[idx], alpha);
                ctx.fill();
            } else if (heat < -0.02) {
                const baseAlpha =
                    Math.min(0.85, 0.2 + Math.abs(heat) * 1.2) *
                    (0.5 + depth * 0.5);
                const alpha =
                    idx === focusDirIdx
                        ? baseAlpha
                        : baseAlpha * (0.6 + 0.4 * focusProx);
                const radius =
                    (0.45 + Math.abs(heat) + depth * 0.3) *
                    (0.5 + 0.5 * focusMul);
                ctx.beginPath();
                ctx.arc(pr.x, pr.y, radius, 0, TAU);
                ctx.fillStyle = `rgba(18,22,34,${(alpha * focusMul).toFixed(3)})`;
                ctx.fill();
            } else {
                const alpha =
                    (0.12 + depth * 0.25 + magnitude * 0.3) * focusMul;
                ctx.beginPath();
                ctx.arc(pr.x, pr.y, 0.4 + depth * 0.45, 0, TAU);
                ctx.fillStyle = `rgba(${sphereRGB},${alpha.toFixed(3)})`;
                ctx.fill();
            }
        }
        ...
        for (let rIdx = 0; rIdx < ringPlanes.length; rIdx++) {
            ...
            for (let seg = 0; seg < SPHERE_RING_SEGS; seg++) {
                ...
                const r0 = rotate(
                    {
                        x: p0.x * SPHERE_RADIUS,
                        y: p0.y * SPHERE_RADIUS,
                        z: p0.z * SPHERE_RADIUS,
                    },
                    angleY,
                    pitchVal,
                );
                const r1 = rotate(
                    {
                        x: p1.x * SPHERE_RADIUS,
                        y: p1.y * SPHERE_RADIUS,
                        z: p1.z * SPHERE_RADIUS,
                    },
                    angleY,
                    pitchVal,
                );
                ...
                const invLen = 1 / SPHERE_RADIUS;
                const mid = {
                    x: (r0.x + r1.x) * 0.5 * invLen,
                    y: (r0.y + r1.y) * 0.5 * invLen,
                    z: (r0.z + r1.z) * 0.5 * invLen,
                };
                ...
                if (heat > 0.05) {
                    ...
                    const alpha =
                        Math.min(0.85, baseA + Math.abs(heat)) *
                        (0.4 + (avgDepth / 2) * 0.6) *
                        focusMul;
        ...
        for (let k = 0; k < axes3D.length; k++) {
            ...
            const haloR =
                ((isHi ? S * 0.15 : S * 0.1) + (isHi ? pulse * S * 0.03 : 0)) *
                breathScale;
            ...
            grad.addColorStop(0.0, hexRGBA(color, Math.min(1, baseA * 1.2)));
            grad.addColorStop(0.15, hexRGBA(color, baseA * 0.8));
            grad.addColorStop(0.35, hexRGBA(color, baseA * 0.45));
            grad.addColorStop(0.6, hexRGBA(color, baseA * 0.18));
            grad.addColorStop(0.85, hexRGBA(color, baseA * 0.05));
            grad.addColorStop(1.0, hexRGBA(color, 0));
            ctx.beginPath();
            ctx.arc(pe.x, pe.y, haloR, 0, TAU);
            ctx.fillStyle = grad;
            ctx.fill();
            ctx.beginPath();
            ctx.arc(
                pe.x,
                pe.y,
                (isHi ? 1.8 : 1.2) * (0.7 + 0.6 * breath),
                0,
                TAU,
            );
            ctx.fillStyle = hexRGBA(color, Math.min(1, baseA + 0.25));
            ctx.fill();
        }
        ...
        for (const { i: lIdx, z: lz } of zOrder) {
            ...
            for (let ll = 0; ll < lines.length; ll++)
                ctx.fillText(
                    lines[ll],
                    labX,
                    labY + (ll - (lines.length - 1) / 2) * lineH,
                );
        }
        ...
        ctx.fillStyle = "rgba(30,58,107,0.35)";
        ctx.fill();

        // Ping animation when pinned
        if (
            pinnedIdx !== null &&
            pinnedIdx < potentialEnds.length &&
            pinStartedAt !== null
        ) {
            ...
            if (t < 1) {
                ctx.beginPath();
                ctx.arc(pe.x, pe.y, t * PING_MAX_R, 0, TAU);
    ...
    function findHit(px, py) {
        let bestIdx = null,
            bestDist = HIT_RADIUS_PX * HIT_RADIUS_PX;
    ...
    function handlePointerMove(e) {
        ...
        if (dragging && dragPrev) {
            const dx = px - dragPrev.x,
                dy = py - dragPrev.y;
            if (dragStart) {
                const tdx = px - dragStart.x,
                    tdy = py - dragStart.y;
                if (Math.hypot(tdx, tdy) > 4) dragMoved = true;
            }
            angleY += (dx / rect.width) * TAU;
            const PITCH_MIN = -Math.PI / 2 + 0.08,
                PITCH_MAX = Math.PI / 2 - 0.08;
            pitchVal = Math.max(
                PITCH_MIN,
                Math.min(PITCH_MAX, pitchVal - (dy / rect.height) * Math.PI),
            );
            dragPrev = { x: px, y: py };
            hoveredIdx = null;
    ...
    onMount(() => {
        raf = requestAnimationFrame(draw);
        const onKey = (e) => {
            if (e.key === "Escape" && pinnedIdx !== null) {
                pinStartedAt = null;
                pinnedIdx = null;
            }
        };
